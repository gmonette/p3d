...
## p3d:Fit3d.R
## 2011-12-22
## Backup old version for new version with stanfit method
...
Fit3d_bak <-
  function(fit,
           ...
           grid = TRUE ,
           base.grid = FALSE,
           # draw grid in base horizontal plane
           col.grid = col,
           col.res = col,
           residuals = FALSE,
           ...
           FUN = function (x)
             x,
           # inverse of transfomation of Y in model,
    ...
    if (residuals) {
      ...
      if (has.groups) {
        ...
        rgl.lines(
          ...
          color = rep(col[as.factor(gg)], each = 2),
